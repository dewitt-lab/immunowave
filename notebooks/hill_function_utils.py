...
def tissue_response(B0, KD, t_max, shape, lb, h, L, hill_coefficient=2, D=1, gamma=1, scale=0.5, **kwargs):
    model = HillModel(KD=KD, n=hill_coefficient, η=1.0, ξ=0.0, λ=0.0, μ=0.0, D=D, gamma=gamma)
    state = State4(
        A=iw.ScalarField(shape, lb, h, 0),
        B=iw.ScalarField(
            shape, lb, h, fn=lambda x: B0 * jax.scipy.stats.norm.pdf(x, loc=L / 2, scale=scale)
        ),
        #B=iw.ScalarField(
...
def single_cell_response(B0, KD, t_max, shape, lb, h, L, hill_coefficient=2, gamma=1.0, **kwargs):
    ...
    model = HillModel(KD=KD, n=hill_coefficient, η=1.0, ξ=0.0, λ=0.0, μ=0.0, D=0.0, gamma=gamma)
    state = State4(
...
def compute_wave_threshold(B0_grid,  KD, t_max, shape, lb, h, L, hill_coefficient=2, D=1, gamma=1, n_iters=5, scale=0.5, **kwargs):
    ...
    with Pool(processes=10) as pool:
        func = partial(tissue_response, KD=KD, t_max=t_max, 
                       ...
                       lb=lb, h=h, L=L, scale=scale, **kwargs)    
        res = pool.map(func, B0_grid)
    final_integrated_A = np.array(res)
    this_id = np.where(np.diff(final_integrated_A) == np.max(np.diff(final_integrated_A)))[0][0]
    if this_id < len(B0_grid) - 1:
        linear_B0_grid = np.linspace(B0_grid[this_id], B0_grid[this_id  + 1], len(B0_grid))
    ...
    for i in range(n_iters):
        ...
        this_id = np.where(np.diff(final_integrated_A) == np.max(np.diff(final_integrated_A)))[0][0]
        if this_id < len(B0_grid) - 1:
            linear_B0_grid = np.linspace(linear_B0_grid[this_id], linear_B0_grid[this_id + 1], len(linear_B0_grid))
        else:
            # initial grid does not contain true threshold, abort
            raise ValueError('initial B0_grid lower bound too high')
    wave_threshold, wave_threshold_uncertainty = find_B0(final_integrated_A, linear_B0_grid)
    
    return wave_threshold, wave_threshold_uncertainty
    

def compute_cell_threshold(B0_grid,  KD, t_max, shape, lb, h, L, hill_coefficient=2, D=1, gamma=1, n_iters=10, **kwargs):
    """do a two-step grid search. First on user-input B0_grid (typically logarithmically spaced).
    Then, find the grid points that bound the threshold and do a second search on a linear grid 
    using the same number of data points as in the original B0_grid."""
    with Pool(processes=10) as pool:
