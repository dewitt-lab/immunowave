...
def test_diffusion_integration(shape, h):
    ...
    if ndim == 1:
        ...
        init_fn = lambda x, y: jnp.exp(-(((x - midpoint[0]) / sigma) ** 2 + 
                                          ((y - midpoint[1]) / sigma) ** 2))
    
    test_state = TestState(
        spatial.ScalarField(shape, lb, h, fn=init_fn)
    )
    ...
    solution = dx.diffeqsolve(
        pde_term, cn_solver, t0, t1, dt0, test_state,
...
def test_conservation_properties_crank_nicolson():
    ...
    class TransportModel(model.Model):
        ...
        def __call__(self, t, state, args=None):
            ...
            return TransportState(
                spatial.ScalarField(density.shape, 
                                    state.density.lb, 
                                    state.density.h, 
                                    values=d_density_dt)
            )
    ...
    field = spatial.ScalarField(
        shape, lb, h, 
        fn=lambda x: jnp.exp(-100 * (x - lb[0] - (shape[0] * h) / 2)**2)
    )
    ...
    solution = dx.diffeqsolve(
        pde_term, cn_solver, t0, t1, dt0, initial_state,
