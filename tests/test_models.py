...
@pytest.mark.parametrize(params_str, params[:1])  # Use only 1D for differentiation tests
...
def test_solve_fhn_model(shape, lb, h):
    ...
    solution = model.solve(
        test_model, test_state, t0, t1, dt0=dt0, rtol=1e-4, atol=1e-4,
...
def test_solve_with_different_controllers():
    ...
    shape, lb, h = (20,), [0], 0.1
    test_state = TestState(
        spatial.ScalarField(shape, lb, h, fn=lambda x: 0.5 * jnp.exp(-(x-1.0)**2)),
    ...
    solution2 = model.solve(
        test_model, test_state, t0, t1, 
        dt0=1e-3, stepsize_controller=pid_controller
    )
...
def test_conservation_properties(shape, lb, h):
    ...
    field = spatial.ScalarField(
        shape, lb, h, 
        fn=lambda x: jnp.exp(-50 * (x - lb[0] - (shape[0] * h) / 2)**2)
    )
    ...
    t0, t1 = 0.0, 0.1
    solution = model.solve(
        test_model, initial_state, t0, t1, dt0=1e-3,
        saveat=dx.SaveAt(t1=True, dense=True)
    )
